...
lugar <- get_map("Bar Minuto, Portugalete, spain", zoom=16, source="stamen", maptype="watercolor")#, source="stamen", maptype = "watercolor")
...
for (i in 1:3){
  #i<-4
   ...
   q<-ggmap(lugar, base_layer=ggplot(lugares[i, ], aes(lon, lat))) + 
               geom_point(aes(color=orden), size = 7)  +
               geom_text(aes(label=orden), color="white") +
 # geom_path(data=trek_df[[1]], aes(lon, lat), size=3) +
    geom_label(aes(label=nombre), fill="tan1", size=5, color="white")  +
